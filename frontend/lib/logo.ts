...
export function getTickerInitials(ticker: string, name?: string): string {
  ...
  // Dual-class share special handling (e.g. BRK.A / BRK-B -> BRK)
...
export function getLogoCandidates(ticker: string): string[] {
  ...
  if (rawTicker.includes(".") || rawTicker.includes("-")) {
    ...
    candidates.push(`https://assets.parqet.com/logos/symbol/${encodeURIComponent(solidTicker)}?format=png`);
    candidates.push(`https://financialmodelingprep.com/image-stock/${encodeURIComponent(solidTicker)}.png`);
  }

  // Domain-based fallback when domain is known
  if (domain) {
    candidates.push(
